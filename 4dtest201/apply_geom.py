...
data_folder = Path("/home/bvermeulen/seismic_unix/4dtest201/data")
rps_fname = data_folder / "sps" / "4dtest_r201.r"
sps_fname = data_folder / "sps" / "4dtest_r201.s"
xps_fname = data_folder / "sps" / "4dtest_r201.x"
geometry_fname = data_folder / "output" / "4dtest201_geometry.txt"

nrcv = 201           # total number of receivers
nsrc = 401           # total number of source points
ttrc = 80_601        # total number of traces
ntrc = ttrc // nsrc  # traces per source point
r_step = 8
first_rcv = 5465
# bin_size_factor: 0.5 = 12.5m, 0.25 = 25m, 0.125=50m
bin_size_factor = 0.25
first_bin = 1001
last_bin = 1400
...
cdp_count = {cdp: 0 for cdp in range(first_bin - 1, last_bin + 2)}
for i in range(nsrc):
    for j in range(ntrc):
        if (
            ...
            cdp = int(((ep_val+rp_val)*0.5 - first_rcv) * bin_size_factor) + first_bin
            cdp_count[cdp] += 1
            write_record.send(
                f"{ep_val:6} "
                f"{sx_val:10.1f} "
