...
do
	...
	sucdpbin < $basefolder/line201filter.su \
		  xline=699440.0,709440.0 \
		  yline=3390584.1,3390584.4\
          verbose=2 dcdp=$dcdp distmax=$distmax 2>$basefolder/cdp.log |
	suwind key=cdp min=1001 > $basefolder/line201cdp.su
	susort < $basefolder/line201cdp.su cdp offset > $basefolder/tmp.su
	suchw < $basefolder/tmp.su key1=d2 a=100.00 b=0 |
	sumute key=offset xmute=-5001,0,5000 tmute=0,0,0 > $basefolder/line201cdp_muted.su
	rm $basefolder/tmp.su

	echo Creating chart data
	suchart < $basefolder/line201cdp_muted.su key1=cdp key2=offset > $basefolder/plotdata outpar=$basefolder/par
