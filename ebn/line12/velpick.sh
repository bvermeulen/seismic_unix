...
#------------------------------------------------
    ...
    sugain tpow=$gain mark=0 > panel.$picknow
...
#------------------------------------------------
    ...
    suwind < $indata key=cdp min=$s max=$k |
    sugain tpow=$gain mark=0 > tmp0
...
#------------------------------------------------
	...
	suximage title="CMP gather $picknow after NMO" xbox=10 ybox=10 \
		wbox=400 hbox=600 verbose=0 f2=$foffset d2=120 perc=95 &
